...
document.getElementById("bmxManubrio").addEventListener("click", function () {
  ...
  ChangeManubrio("bmx/bmxManubrio.glb", {
    position: new BABYLON.Vector3(0, 4.87, -3),
...
document.getElementById("corsaManubrio").addEventListener("click", function () {
  ...
  ChangeManubrio("corsa/corsaManubrioTest.glb", {
    position: new BABYLON.Vector3(0, 3.95, -3.55),
    rotation: new BABYLON.Vector3(0, 0, 0),
...
async function changeWheels(pathNuovaRuota, btn) {
  ...
  nuovaRuotaR.meshes.forEach((mesh) => {
    mesh.setAbsolutePosition(new BABYLON.Vector3(0, -0.1, -2.15));
  });
  nuovaRuotaF.meshes.forEach((mesh) => {
    mesh.setAbsolutePosition(new BABYLON.Vector3(0, -0.05, 1.55));
  });
...
window.aggiungiPortaTelefono = async function () {
  if (
    window.portaTelefonoMesh &&
    window.portaTelefonoMesh.isDisposed() === false
  ) {
...
function selectWheelButton(colorClass) {
  document
    .querySelectorAll(".tireColorSelector-wrapper button")
    .forEach((btn) => {
...
function selectTelaioButton(colorClass) {
  document
    .querySelectorAll(".telaioColorSelector-wrapper button")
    .forEach((btn) => {
      if (btn.classList.contains(colorClass)) {
        btn.classList.add("selected");
      } else {
...
function selectSaddleButton(colorClass) {
  document
    .querySelectorAll(".saddleColorSelector-wrapper button")
    .forEach((btn) => {
      if (btn.classList.contains(colorClass)) {
        btn.classList.add("selected");
      } else {
...
function selectButtonAccessori(colorClass) {
  document
    .querySelectorAll(".AccessoriColorSelector-wrapper button")
    .forEach((btn) => {
      if (btn.classList.contains(colorClass)) {
        btn.classList.add("selected");
      } else {
...
function changeColorAccessori(colorName) {
  ...
  if (window.portaTelefonoMesh && window.portaTelefonoMesh.getChildMeshes) {
    const matPortaTelefono = new BABYLON.StandardMaterial(
      "matPortaTelefono",
      scene
    );
...
const createScene = async () => {
  ...
  async function changeSaddle(pathNuovaSaddle, btn) {
    ...
    let selleImportate = nuovaSaddle.meshes.filter((mesh) =>
      mesh.name.toLowerCase().includes("sella")
    );
  ...
  const body = bikeResult.meshes.filter((mesh) =>
  ...
  ruote = bikeResult.meshes.filter((mesh) =>
    mesh.name.toLowerCase().includes("ruota")
  );

  manubrio = bikeResult.meshes.filter((mesh) =>
    mesh.name.toLowerCase().includes("manubrio")
  );

  // Imposta automaticamente onload gli elementi per la bici da mountain bike
  await changeWheels("bmx/bmxRuota2.glb");
...
salvaPrezzoTotale = () => {
  ...
  let configurazione = {
    ...
    accessori: {
      portaTelefono:
        !!window.portaTelefonoMesh && !window.portaTelefonoMesh.isDisposed(),
      portaborraccia:
        !!window.borracciaMesh && !window.borracciaMesh.isDisposed(),
    },
